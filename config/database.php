...
"CREATE TABLE IF NOT EXISTS metalcolor_sessions (token_hash TEXT PRIMARY KEY,user_id BIGINT NOT NULL REFERENCES metalcolor_users(id) ON DELETE CASCADE,expires_at TIMESTAMPTZ NOT NULL,revoked_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())",
