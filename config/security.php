...
function mc_is_production(): bool {
    return strtolower((string)(getenv('APP_ENV') ?: getenv('VERCEL_ENV') ?: '')) === 'production';
}

function mc_secret(): string {
    $v=(string)(getenv('RATE_LIMIT_SECRET')?:'');
    if(strlen($v)>=32)return $v;
    if(mc_is_production())throw new RuntimeException('RATE_LIMIT_SECRET ausente ou muito curto.');
...
function mc_valid_uuid(string $v): bool { return preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i',$v)===1; }
...
function mc_validate_core_secrets(): void {
    if(!mc_is_production())return;
    foreach(['RATE_LIMIT_SECRET','JWT_SECRET'] as $n){
        $v=(string)getenv($n);
        if(strlen($v)<24||stripos($v,'troque-')!==false||stripos($v,'aleatorio')!==false||stripos($v,'dev-only')!==false){
            throw new RuntimeException("$n ausente ou fraco para produção.");
        }
    }
}

function mc_validate_admin_bootstrap(): void {
    if(!mc_is_production())return;
    $email=(string)getenv('ADMIN_EMAIL');$password=(string)getenv('ADMIN_PASSWORD');
    if($email!=='' && (!mc_valid_email($email,false) || strlen($password)<12)) {
        throw new RuntimeException('Credenciais iniciais do administrador inválidas.');
    }
}

function mc_validate_payment_config(): void {
    mc_validate_core_secrets();
    $api=(string)getenv('ASAAS_API_KEY');
    if(strlen($api)<20)throw new RuntimeException('ASAAS_API_KEY ausente ou inválida.');
    mc_commission_rate();

    if(strtolower((string)(getenv('ASAAS_ENVIRONMENT')?:'sandbox'))==='production'){
        $web=(string)getenv('ASAAS_WEBHOOK_TOKEN');
