(function(){
  ...
  async function restore(){if(read().length)return read();try{const r=await fetch('/api/account/cart');if(!r.ok)return read();const d=await r.json();if(Array.isArray(d.cart)&&d.cart.length){localStorage.setItem(KEY,JSON.stringify(d.cart));window.dispatchEvent(new CustomEvent('metalcolor:cart',{detail:d.cart}));return d.cart}}catch{}return read()}
