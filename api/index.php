...
case '/api/auth/register':
    mc_require_method('POST'); mc_same_origin(); mc_validate_production_secrets(); $b=mc_body();$name=cleanv($b['name']??'',100);$email=strtolower(cleanv($b['email']??'',160));$password=(string)($b['password']??'');
    if(!mc_rate_limit($pdo,'register',6,3600,$email))mc_json(429,['error'=>'Muitas tentativas de cadastro. Tente novamente mais tarde.']);
    if(mb_strlen($name)<2||!mc_valid_email($email,false)||strlen($password)<10||strlen($password)>200)mc_json(400,['error'=>'Informe nome, e-mail válido e senha com pelo menos 10 caracteres.']);
